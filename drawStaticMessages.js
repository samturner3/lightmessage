...
module.exports = function drawStaticMessages() {
  ...
  if (globalMode.messagesStatic.length > 0) {
    globalMode.messagesStatic.forEach((item) => {
      globalMode.buffer.push(new BufferItem(item.x, item.y, item.message, fonts.fontFiles[item.font], item.r, item.g, item.b));
    })
  }
